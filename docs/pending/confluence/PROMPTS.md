...
### 3.8 Dominio de tareas — CRUD endpoints, use cases y alineación de contrato de reportes (PMCP-26/50)

**Fecha**: 2026-05-06
**Modelo**: claude-sonnet-4-6
**Resultado**: Implementación completa del dominio de tareas (CreateTaskUseCase, UpdateTaskUseCase, tasks_router con 6 endpoints y DI) y corrección del contrato frontend/backend de reportes. Code review integrado con 4 correcciones bloqueantes aplicadas.
**Ficheros generados/afectados**:
- `backend/src/application/use_cases/create_task_use_case.py`
- `backend/src/application/use_cases/update_task_use_case.py`
- `backend/src/application/commands/update_task_command.py`
- `backend/src/adapters/primary/api/tasks_router.py`
- `backend/src/infrastructure/container.py`
- `backend/src/adapters/primary/api/reports_router.py`
- `frontend/src/services/reportService.ts`
- `backend/tests/unit/application/use_cases/test_create_task_use_case.py`
- `backend/tests/unit/application/use_cases/test_update_task_use_case.py`
- `backend/tests/unit/adapters/primary/test_tasks_router.py`

**Prompt**:

```
Revisa el contexto del proyecto, aplica el skill session-start para ver por dónde continuamos
[confirmación: ok]
[confirmación: sí — contrato de reportes]
[lanza los skills de code-reviewer, confluence-publisher, docs-updater, prompt-librarian y test-writer para actualizar documentación, promts relevantes, jiras y tests]
```

**Notas**:
- El patrón de DI en tasks_router sigue exactamente el de projects_router: funciones `get_X()` que retornan `container.X`, sobrescribibles en tests con `app.dependency_overrides[get_X] = lambda: mock`.
- `UpdateTaskCommand` en `application/commands/` (no inline en el use case) es la convención del proyecto — el code-reviewer lo detectó como inconsistencia.
- `model_dump(exclude_unset=True)` es el patrón correcto para PATCH en Pydantic v2 — permite borrar campos nullable enviando `null` explícitamente, sin afectar campos no enviados.
- El bug `jira=` vs `jira_port=` en el container fue detectado por code-reviewer (habría causado TypeError en runtime al primer uso de la integración Jira).
- `datetime.utcnow()` está deprecado en Python 3.12 — usar `datetime.now(tz=timezone.utc)` produce objetos aware que requieren comparaciones con otros objetos aware en los tests.
- Reutilizable en: cualquier CRUD hexagonal en FastAPI — el patrón router → CreateUseCase/UpdateUseCase → RepositoryPort es el estándar del proyecto.
- No usar cuando: la entidad tiene reglas de transición de estado complejas (usar un método de dominio específico en lugar de `setattr` genérico).
...
### 5.6 Tests para dominio de tareas — CreateTaskUseCase, UpdateTaskUseCase y tasks_router (PMCP-26)

**Fecha**: 2026-05-06
**Modelo**: claude-sonnet-4-6
**Resultado**: 27 tests nuevos para el dominio de tareas; corrección del test `test_updates_updated_at` por incompatibilidad naive/aware datetime al migrar a `timezone.utc`.
**Ficheros generados/afectados**:
- `backend/tests/unit/application/use_cases/test_create_task_use_case.py`
- `backend/tests/unit/application/use_cases/test_update_task_use_case.py`
- `backend/tests/unit/adapters/primary/test_tasks_router.py`
...
```
[Tras implementar el dominio de tareas — aplicar skill test-writer]
Escribe tests para CreateTaskUseCase (happy path, jira_sync_status, field mapping, UUID único, repo call),
UpdateTaskUseCase (status, multiple fields, not found → DomainError, unknown fields, save call,
updated_at timezone-aware, empty updates) y tasks_router (list, create, get, patch, delete, sync-jira
con todos los casos de error: 503 sin Jira, 404 tarea no encontrada, 422 sin jira_project_key).
```

**Notas**:
- El test `test_updates_updated_at` falló tras migrar a `datetime.now(tz=timezone.utc)` porque `sample_task.updated_at` es naive (usa `datetime.utcnow()` en la fixture). Solución: no comparar con el valor original, sino verificar que `tzinfo is not None` y `tzinfo == timezone.utc`.
- Para los tests del router de tasks, los overrides de DI se deben limpiar con `app.dependency_overrides.pop(key, None)` después de cada test (no `clear()` para no romper otros tests en paralelo).
- El `_mock_update_uc(raises=DomainError(...))` permite testear que el router convierte `DomainError` en 404 sin depender de la implementación real del use case.
- Reutilizable en: cualquier router FastAPI con el mismo patrón de DI — copiar la estructura de `_mock_task_repo`, `_mock_create_uc`, `_mock_update_uc` y adaptar los fixtures.
