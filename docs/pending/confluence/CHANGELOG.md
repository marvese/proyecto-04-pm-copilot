...
- **PMCP-33**: `JiraAdapter` implementado — `get_issue()`, `create_issue()`, `update_issue()`, `list_issues()` con JQL, `health_check()`; todos los métodos de escritura/lectura protegidos con tenacity (3 intentos, backoff exponencial, reraise); `health_check()` usa bare try/except (sin retry intencional, es una probe)
- **PMCP-34**: `ConfluenceAdapter` implementado — `get_page()`, `create_page()` (con parent opcional), `update_page()` (auto-incrementa versión), `list_pages()`, `get_page_content()`, `health_check()`; función `markdown_to_storage()` portada de `scripts/confluence_client.py` e integrada directamente en el adaptador
- **PMCP-35**: `GitHubAdapter` implementado — `list_pull_requests()`, `get_pull_request()`, `list_issues()`, `get_file_content()` (decodificación base64 del contenido del repositorio), `health_check()`; tenacity en todos los métodos públicos
- **PMCP-36**: `SyncJiraUseCase` implementado — `execute(project_id, jira_project_key)` lista tareas PENDING, crea issue en Jira si `jira_key is None` o actualiza si ya existe, marca `SYNCED`/`FAILED` y persiste; continúa tras fallos parciales; `sync_single()` para sync inmediato de una tarea individual; `SyncJiraResult` dataclass con contadores. Suite total: **157 unitarios + 18 integración = 175 passed, 4 skipped**
